import { ref } from 'vue';
import { defineStore } from 'pinia';

export const useUserStore = defineStore('user', () => {
  const user = ref({});
  const jwt = ref('');
  async function getToken() {
    user.value = {
      id: '92409511',
      name: 'wayhuang',
      empNameE: 'Wayne T. Huang',
      empNameC: '黃尊昱',
      phone: '2960',
      divCd: 'SS',
      rankCd: '50',
      lmu: 'SI15',
      role: 9
    };
    jwt.value =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..eDAXYiBLKVc_Kgclpz731AxuDPXcoiIqvEyapbJW2rw';

    /*     {
      "user": {
          "id": "92409511",
          "name": "wayhuang",
          "empNameE": "Wayne T. Huang",
          "empNameC": "黃尊昱",
          "phone": "2960",
          "divCd": "SS",
          "rankCd": "50",
          "lmu": "SI15",
          "role": 9
      },
      "jwt": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..eDAXYiBLKVc_Kgclpz731AxuDPXcoiIqvEyapbJW2rw"
  } */
  }

  return { user, jwt, getToken };
});
